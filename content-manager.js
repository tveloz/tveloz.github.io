...
class ContentManager {
    ...
    saveConfig(config) {
        try {
            ...
            config.metadata.total_blog_posts = config.content.blog_posts?.length || 0;
    ...
    getContentByTags(tags, type = 'all') {
        ...
        if (type === 'all' || type === 'blog') {
            content = content.concat(config.content.blog_posts || []);
        }
    ...
    updateMainCarousel() {
        try {
            ...
            console.log('✅ Main carousel updated - now uses dynamic content.json loading');
            console.log('📝 No changes needed to index.html - content loads from content.json automatically');
            
            // Since the new index.html loads content.json dynamically,
            // we just need to ensure content.json is up to date
            const config = this.loadConfig();
            this.saveConfig(config); // This updates the metadata
    ...
    addContent(contentData) {
        ...
        } else if (contentData.type === 'blog') {
            config.content.blog_posts = config.content.blog_posts || [];
            config.content.blog_posts.push(newContent);
        }
    ...
    removeContent(contentId) {
        ...
        // Check blog posts
        if (config.content.blog_posts) {
            const blogIndex = config.content.blog_posts.findIndex(item => item.id === contentId);
            if (blogIndex !== -1) {
                config.content.blog_posts.splice(blogIndex, 1);
                removed = true;
            }
        }
    ...
    getStats() {
        ...
        return {
            ...
            blog_posts: config.content.blog_posts?.length || 0,
            carousels: Object.keys(config.carousels).length,
            total_content: config.content.photos.length + config.content.videos.length + (config.content.publications?.length || 0) + (config.content.blog_posts?.length || 0),
...
Commands:
  ...
  add-blog                     - Add new blog post (interactive)
